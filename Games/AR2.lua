...
local function safeGetUpvalue(fn, idx, name)
    local ok, val = pcall(getupvalue, fn, idx)
    if not ok or val == nil then
        warn("failed to init \"" .. name .. "\" (getupvalue " .. idx .. " failed)")
        return nil
    end
    return val
end

local Events = safeGetUpvalue(Network.Add, 1, "Events")
if type(Events) == "function" then
    Events = safeGetUpvalue(Network.Add, 2, "Events")
end

local GetSpreadAngle = safeGetUpvalue(Bullets.Fire, 1, "No Spread")
local GetSpreadVector = safeGetUpvalue(Bullets.Fire, 3, "SpreadVector")
local CastLocalBullet = safeGetUpvalue(Bullets.Fire, 4, "Bullet Tracer")
local GetFireImpulse = safeGetUpvalue(Bullets.Fire, 6, "Recoil Control")
local LightingState = safeGetUpvalue(Lighting.GetState, 1, "Lighting")
local AnimatedReload = safeGetUpvalue(Firearm, 7, "Instant Reload")

local SetWheelSpeeds = safeGetUpvalue(VehicleController.Step, 2, "Vehicle Speed")
local SetSteerWheels = safeGetUpvalue(VehicleController.Step, 3, "Vehicle Steer")

local Effects = CastLocalBullet and safeGetUpvalue(CastLocalBullet, 2, "Effects") or nil
local Sounds = CastLocalBullet and safeGetUpvalue(CastLocalBullet, 3, "Sounds") or nil
local ImpactEffects = CastLocalBullet and safeGetUpvalue(CastLocalBullet, 6, "Bullet Tracer ImpactEffects") or nil
...
if not InteractHeartbeat or not FindItemData then
    warn("failed to init \"Instant Search\" (InteractHeartbeat not found)")
end
...
local OldBaseTime = LightingState and LightingState.BaseTime or 0
...
local Window = Radical.Utilities.UI:Window({
    ...
    local MiscTab = Window:Tab({Name = "Miscellaneous"}) do local LModes = {}
        local LightingSection = MiscTab:Section({Name = "Lighting", Side = "Left"}) do
            ...
            Callback = function(Bool) if not Bool and LightingState then LightingState.BaseTime = OldBaseTime end end})
            LightingSection:Slider({Name = "Time", Flag = "AR2/Lighting/Time", Min = 0, Max = 24, Precise = 1, Value = 12, Unit = "hours"})
            local ok, lightingModes = pcall(getupvalue, Lighting.GetState, 4)
            if ok and lightingModes then
                for Name, LightingMode in pairs(lightingModes) do
                    LModes[#LModes + 1] = {Name = Name, Mode = "Button", Value = false,
                    Callback = function() Lighting:SetMode(Name) end}
                end
            else
                warn("failed to init \"Lighting Modes\" (upvalue 4 not found)")
            end
...
WallCheckParams.FilterDescendantsInstances = {Workspace.Effects}
WallCheckParams.IgnoreWater = true

local function Raycast(Origin, Direction)
    if not table.find(WallCheckParams.FilterDescendantsInstances, LocalPlayer.Character) then
        WallCheckParams.FilterDescendantsInstances = {Workspace.Effects, LocalPlayer.Character}
    end
...
local function InEnemyTeam(Enabled, Player)
    ...
    if SquadData and SquadData.Members then
        if table.find(SquadData.Members, Player.Name) then return false end
    end
    return true
end
...
local function AimAt(Hitbox, Sensitivity)
    ...
    mousemoverel((Hitbox[4].X - MouseLocation.X) * Sensitivity, (Hitbox[4].Y - MouseLocation.Y) * Sensitivity)
end
...
local function GetStates()
    ...
    local Data = {"ServerTime", "RootCFrame", "RootVelocity", "FirstPerson", "InstanceCFrame", "LookDirection", "MoveState", "AtEaseInput", "ShoulderSwapped", "Zooming", "BinocsActive", "Staggered", "Shoving"}
...
local function CastLocalBulletInstant(Origin, Direction, SpreadDirection)
    ...
    local Exclude = {Effects, Sounds, PlayerClass.Character.Instance}
    while true do
        TravelTime += Frame
        ...
        if ProjectileCastInstance or TravelDistance > ShotMaxDistance then break end
    end
...
local function CIIC(Data)
    ...
    for Index, Value in pairs(Data.Occupants) do
        if Duplicates[Value.Name] then Duplicates[Value.Name] += 1
        else Duplicates[Value.Name] = 1 end
    end
    for Item, Value in pairs(Duplicates) do
        Items[#Items + 1] = Value == 1 and "[" .. Item .. "]" or "[" .. Item .. "] x" .. Value
    end
...
local function HookCharacter(Character)
    ...
    if not ok then warn("failed to init \"Equip Hook\": " .. tostring(err)) end

    ok, err = pcall(function()
        local OldJump; OldJump = hookfunction(Character.Actions.Jump, newcclosure(function(Self, ...)
            local Args = {...}
            if Window.Flags["AR2/NoJumpDebounce"] then Self.JumpDebounce = 0 end
            if Args[1] == "Begin" and Window.Flags["AR2/JumpHeight/Enabled"] then
                local ReturnArgs = {OldJump(Self, ...)}
                if Self.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and not Window.Flags["AR2/JumpHeight/NoFallCheck"] then return end
    ...
    if not ok then warn("failed to init \"Jump Height / No Jump Debounce\": " .. tostring(err)) end
    ...
    if not ok then warn("failed to init \"Unlock Firemodes\": " .. tostring(err)) end
end
...
if not ok then warn("failed to init \"Head Expander __index\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Crash Bypass __namecall\": " .. tostring(err)) end
...
ok, err = pcall(function()
    local OldSend; OldSend = hookfunction(Network.Send, newcclosure(function(Self, Name, ...)
        ...
        if Name == "Vehicle Bumper Impact" and Window.Flags["AR2/Vehicle/Impact"] then return end
        if Name == "Inventory Container Group Disconnect" and Window.Flags["AR2/ContainerPersistence"] then return end
        return OldSend(Self, Name, ...)
    end))
end)
if not ok then warn("failed to init \"Network.Send Hook\": " .. tostring(err)) end
        ...
        if Name == "Character State Report" then
            ...
            for Index = 1, #Args do
                if Window.Flags["AR2/SSCS"] then
                    if RandomData[Index] == "MoveState" then Args[Index] = Window.Flags["AR2/MoveState"][1] end
                end
                if Window.Flags["AR2/NoSpread"] then
                    if RandomData[Index] == "Zooming" then Args[Index] = true
                    elseif RandomData[Index] == "FirstPerson" then Args[Index] = true end
                end
            end
...
if not ok then warn("failed to init \"Network.Fetch Hook / Spoof State / No Spread\": " .. tostring(err)) end

ok, err = pcall(function()
    if not GetSpreadAngle then error("GetSpreadAngle is nil") end
...
if not ok then warn("failed to init \"No Spread (setupvalue)\": " .. tostring(err)) end

ok, err = pcall(function()
    if not ImpactEffects then error("ImpactEffects is nil") end
...
if not ok then warn("failed to init \"Bullet Tracer\": " .. tostring(err)) end

ok, err = pcall(function()
    if not GetFireImpulse then error("GetFireImpulse is nil") end
...
if not ok then warn("failed to init \"Recoil Control\": " .. tostring(err)) end

ok, err = pcall(function()
    if not SetWheelSpeeds then error("SetWheelSpeeds is nil") end
    setupvalue(VehicleController.Step, 2, function(Self, Throttle, ...)
        if Window.Flags["AR2/Vehicle/Enabled"] then
            if not PlayerClass.Character then return end
            Throttle = Window.Flags["AR2/Vehicle/Instant"] and PlayerClass.Character.MoveVector.Z or -Throttle
...
if not ok then warn("failed to init \"Vehicle Speed\": " .. tostring(err)) end

ok, err = pcall(function()
    if not SetSteerWheels then error("SetSteerWheels is nil") end
    setupvalue(VehicleController.Step, 3, function(Self, Steer, Throttle, ...)
        if Window.Flags["AR2/Vehicle/Enabled"] then
            if not PlayerClass.Character then return end
            Steer = Window.Flags["AR2/Vehicle/Instant"] and -PlayerClass.Character.MoveVector.X or -Steer
            for Index, Wheel in pairs(Self.Wheels:GetChildren()) do
                ...
                if Attachment then Angle += math.rad(Attachment.Orientation.Y) end
...
if not ok then warn("failed to init \"Vehicle Steer\": " .. tostring(err)) end

ok, err = pcall(function()
    if not AnimatedReload then error("AnimatedReload is nil") end
    setupvalue(Firearm, 7, function(...)
        if Window.Flags["AR2/InstantReload"] then
            local Args = {...}
            for Index = 0, Args[3].LoopCount do Args[4]("Commit", "Load") end
            Args[4]("Commit", "End")
            return true
        end
...
if not ok then warn("failed to init \"Instant Reload\": " .. tostring(err)) end

ok, err = pcall(function()
    if not InteractHeartbeat or not FindItemData then error("InteractHeartbeat or FindItemData is nil") end
...
if not ok then warn("failed to init \"Instant Search\": " .. tostring(err)) end
...
ok, err = pcall(function()
    local OldFire; OldFire = hookfunction(Bullets.Fire, newcclosure(function(Self, ...)
        if SilentAim and math.random(100) <= Window.Flags["SilentAim/HitChance"] then
            ...
            BodyPartPosition = SolveTrajectory(BodyPartPosition, BodyPart.AssemblyLinearVelocity, Direction.Magnitude / ProjectileSpeed, ProjectileGravity)
...
if not ok then warn("failed to init \"Silent Aim\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"No Camera Flinch\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Use In Air/Water\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"No Fall Impact\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Fast Respawn\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Lighting State Update\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Squad Update\": " .. tostring(err)) end
    ...
    local OldICA; OldICA = hookfunction(Events["Inventory Container Added"], newcclosure(function(Id, Data, ...)
        ...
        if Data.Type ~= "Corpse" or Data.Type ~= "Vehicle" then
            if Data.WorldPosition and Length(Data.Occupants) > 0 then
                AddObject:Fire(Data.Id, CIIC(Data), Data.WorldPosition, "AR2/ESP/Items", "AR2/ESP/Items/Containers", Window.Flags)
            end
        end
        return OldICA(Id, Data, ...)
    end))
end)
if not ok then warn("failed to init \"Container ESP (ICA)\": " .. tostring(err)) end
...
if not ok then warn("failed to init \"Container ESP (CC)\": " .. tostring(err)) end
...
Radical.Utilities.NewThreadLoop(0, function()
    ...
    AimAt(GetClosest(Window.Flags["Aimbot/Enabled"], Window.Flags["Aimbot/TeamCheck"], Window.Flags["Aimbot/VisibilityCheck"], Window.Flags["Aimbot/DistanceCheck"], Window.Flags["Aimbot/DistanceLimit"], Window.Flags["Aimbot/FOV/Radius"], Window.Flags["Aimbot/Priority"][1], Window.Flags["Aimbot/BodyParts"], Window.Flags["Aimbot/Prediction"]), Window.Flags["Aimbot/Sensitivity"] / 100)
end)
Radical.Utilities.NewThreadLoop(0, function()
    SilentAim = GetClosest(Window.Flags["SilentAim/Enabled"], Window.Flags["SilentAim/TeamCheck"], Window.Flags["SilentAim/VisibilityCheck"], Window.Flags["SilentAim/DistanceCheck"], Window.Flags["SilentAim/DistanceLimit"], Window.Flags["SilentAim/FOV/Radius"], Window.Flags["SilentAim/Priority"][1], Window.Flags["SilentAim/BodyParts"])
end)
Radical.Utilities.NewThreadLoop(0, function()
    if not (Trigger or Window.Flags["Trigger/AlwaysEnabled"]) then return end
    if not isrbxactive() then return end
    local TriggerClosest = GetClosest(Window.Flags["Trigger/Enabled"], Window.Flags["Trigger/TeamCheck"], Window.Flags["Trigger/VisibilityCheck"], Window.Flags["Trigger/DistanceCheck"], Window.Flags["Trigger/DistanceLimit"], Window.Flags["Trigger/FOV/Radius"], Window.Flags["Trigger/Priority"][1], Window.Flags["Trigger/BodyParts"], Window.Flags["Trigger/Prediction"])
    ...
    if Window.Flags["Trigger/HoldMouseButton"] then
        while task.wait() do
            TriggerClosest = GetClosest(Window.Flags["Trigger/Enabled"], Window.Flags["Trigger/TeamCheck"], Window.Flags["Trigger/VisibilityCheck"], Window.Flags["Trigger/DistanceCheck"], Window.Flags["Trigger/DistanceLimit"], Window.Flags["Trigger/FOV/Radius"], Window.Flags["Trigger/Priority"][1], Window.Flags["Trigger/BodyParts"], Window.Flags["Trigger/Prediction"])
...
Radical.Utilities.NewThreadLoop(0.1, function()
    ...
    local Enemies = GetEnemyForMelee(Window.Flags["AR2/MeleeAura"], Window.Flags["AR2/AntiZombie/MeleeAura"])
