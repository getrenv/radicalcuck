...
local OldGetAnimator = Tortoiseshell.Items.GetAnimator
...
local Window = Radical.Utilities.UI:Window({
    Name = ("Radical Hub %s %s"):format(utf8.char(8212), Radical.Game.Name),
    ...
    local RageTab = Window:Tab({Name = "Rage"}) do
        local AutoshootSection = RageTab:Section({Name = "Rage", Side = "Left"}) do
            ...
            AutoshootSection:Toggle({Name = "Instant Reload", Flag = "BB/Rage/Autoshoot/InstantReload", Value = false}):Keybind()
            local PriorityList, BodyPartsList = {{Name = "Closest", Mode = "Button", Value = true}, {Name = "Random", Mode = "Button"}}, {}
            for Index, Value in pairs(KnownBodyParts) do
                PriorityList[#PriorityList + 1] = {Name = Value[1], Mode = "Button", Value = false}
                BodyPartsList[#BodyPartsList + 1] = {Name = Value[1], Mode = "Toggle", Value = Value[2]}
        ...
        local MiscSection = MiscTab:Section({Name = "Other", Side = "Left"}) do
            MiscSection:Button({Name = "Redeem Codes", Callback = function()
                ...
                if Success then
                    ...
                    Radical.Utilities.UI:Push({Title = "Radical Hub", Description = "All available codes are claimed!", Duration = 5})
                else
                    Radical.Utilities.UI:Push({Title = "Radical Hub", Description = "Failed to get the codes:\n" .. Error, Duration = 5})
                end
            end})
        end
    ...
    end Radical.Utilities:SettingsSection(Window, "RightShift", false)
...
local function ObjectOccluded(Enabled, Origin, Position)
    if not Enabled then return false end
    return Raycast(Origin, Position - Origin,
...
local function CustomizeCharacter(Enabled, HideTextures, Color, Reflectance, Material)
    ...
    for Index, Instance in pairs(CharacterModel:GetDescendants()) do
        if HideTextures and Instance:IsA("Texture") then
            Instance.Transparency = 1
        elseif Instance:IsA("BasePart")
        and Instance.Transparency < 1
...
local function GetReticlePosition(BodyPart)
    repeat task.wait() until CharacterModel:FindFirstChild("Body")
    local ReticlePosition = CharacterModel.Body.Head.Position
    local LookVector = (BodyPart.Position - ReticlePosition).Unit
...
local function Autoshoot()
    if not AutoshootHitbox then return end
    ...
    if Weapon and (Config and Config.Controller == "Paintball") then
        ...
        if AmmoValue > 0 then
            ...
            RayNormal = ComputeProjectiles(Config, BodyPart)
            ...
            task.spawn(function()
                ...
                for Index, Projectile in pairs(ShootProjectiles) do
                    ...
                    Projectile[2], RayPosition, BodyPart, RayNormal, PlayerCharacter)
                end
            end)

            if Window.Flags["BB/Rage/Autoshoot/InstantReload"] then
                Tortoiseshell.Network:Fire("Item_Paintball", "Reload", Weapon)
            end

            local FireRate = Window.Flags["BB/Rage/Autoshoot/FireRate"]
            task.wait(60 / (CurrentFireMode.FireRate * FireRate))

            if (AmmoValue - Ammo.Value) >= 1 then
                Notify:Fire({
                    Title = ("Autoshoot | Hit %s | Ammo %s"):format(
                        Player.Name, Ammo.Value
                    ), Color = Color3.new(1, 0.5, 0.25), Duration = 3
                })

                if Window.Flags["BB/Rage/Hitmarker"] then
                    Tortoiseshell.UI.Events.Hitmarker:Fire(BodyPart, RayPosition, #ShootProjectiles > 3)
                end
                if Window.Flags["BB/BulletTracer/Enabled"] then
                    Radical.Utilities.MakeBeam(ReticlePosition, RayPosition, Window.Flags["BB/BulletTracer/Color"])
                end
            end
        else
...
local function KnifeAura()
    ...
    if Weapon and (Config and Config.Controller == "Melee") then
        local Origin = CharacterModel.PrimaryPart
        local BodyPart = AutoshootHitbox[3]
        local Player = AutoshootHitbox[1]

        if (BodyPart.Position - Origin.Position).Magnitude <= 15 then
            Tortoiseshell.Network:Fire("Item_Melee", "StabBegin", Weapon)
            Tortoiseshell.Network:Fire("Item_Melee", "Stab", Weapon, BodyPart,
            BodyPart.Position, BodyPart.Position - Origin.Position)

            if Window.Flags["BB/Rage/Hitmarker"] then
                Tortoiseshell.UI.Events.Hitmarker:Fire(BodyPart)
            end
            if Window.Flags["BB/BulletTracer/Enabled"] then
                Radical.Utilities.MakeBeam(Origin.Position, BodyPart.Position, Window.Flags["BB/BulletTracer/Color"])
            end

            Notify:Fire({
                Title = ("Knife Aura | Stab %s"):format(Player.Name),
                Color = Color3.new(1, 0.5, 0.25), Duration = 3
            })
...
local function GetClosestAllFOV(Enabled,
    ...
    for Index, Player in pairs(PlayerService:GetPlayers()) do
        ...
        if Priority == "Random" then
            ...
            if ObjectOccluded(VisibilityCheck, CameraPosition, BodyPartPosition) then continue end
...
local function GetClosest(Enabled, VisibilityCheck, DistanceCheck,
    ...
    if not Enabled then return end
    if not (CharacterModel and CharacterModel.Parent) then return end
    ...
    for Index, Player in ipairs(PlayerService:GetPlayers()) do
        ...
        if Priority == "Random" then
            ...
            BodyPart = GetBodyPart(Body, PriorityPart)
            if not BodyPart then continue end
            ...
            BodyPart.AssemblyLinearVelocity, Distance / ProjectileSpeed, ProjectileGravity, GravityCorrection) or BodyPartPosition
...
Radical.Utilities.FixUpValue(Tortoiseshell.Network.Fire, function(Old, Self, ...)
    local Args = {...}
    ...
    if Args[2] == "Shoot" then
        if SilentAim and math.random(100) <= Window.Flags["SilentAim/HitChance"] then
            task.spawn(function()
                ...
                local PlayerCharacter = SilentAim[2]
                local Weapon, Config = GetEquippedWeapon()
                local ShootProjectiles, ReticlePosition, RayPosition,
                RayNormal = ComputeProjectiles(Config, BodyPart)
                if not ShootProjectiles then return end
                ...
                for Index, Projectile in pairs(ShootProjectiles) do
                    ...
                    RayPosition, BodyPart, RayNormal, PlayerCharacter)
                end
    ...
    elseif Args[2] == "__Hit" then
        if SilentAim or AutoshootHitbox then return Old(Self, ...) end
        if Window.Flags["BB/BulletTracer/Enabled"] then
            local ReticlePosition = Tortoiseshell.Input.Reticle:GetPosition()
            Radical.Utilities.MakeBeam(ReticlePosition, Args[4], Window.Flags["BB/BulletTracer/Color"])
        end
...
Radical.Utilities.FixUpValue(Tortoiseshell.Projectiles.InitProjectile, function(Old, Self, A, B, C, D, ...)
...
Radical.Utilities.FixUpValue(Tortoiseshell.Raycast.CastGeometryAndEnemies, function(Old, Self, ...)
...
Radical.Utilities.FixUpValue(Tortoiseshell.Items.GetAnimator, function(Old, Self, A, B, C, ...)
...
-- Old Config Mod
--[[Radical.Utilities.FixUpValue(Tortoiseshell.Items.GetConfig, function(Old, Self, ...)
...
HeartbeatConnections["Control"] = function(Delta, ...)
    if Window.Flags["BB/Fly/Enabled"] or Window.Flags["BB/AntiAim/Enabled"] then
        if CharacterModel and CharacterModel.Parent then
            local Args = {OldControl(Delta, ...)}

            if Window.Flags["BB/Fly/Enabled"] then
                local MoveVector = Radical.Utilities.MovementToDirection()
                CharacterModel.PrimaryPart.AssemblyLinearVelocity = Vector3.zero
                CharacterModel.PrimaryPart.CFrame += MoveVector * Window.Flags["BB/Fly/Speed"] * (Delta * 60)
            end
            if Window.Flags["BB/AntiAim/Enabled"] then
                ...
                CharacterModel.PrimaryPart.CFrame *= CFrame.Angles(math.rad(180 * Roll), math.rad(180 * Yaw), 0)
            end
...
Radical.Utilities.NewThreadLoop(0.5, function()
    ...
    if Weapon and Config then
        ...
        if Window.Flags["BB/AutoFireMode"] and (Config.FireModeList and Config.FireModes) then
            Config.FireModes = {Auto = {FireRate = Config.FireModes[Config.FireModeList[1]].FireRate}}
            Config.FireModeList = {"Auto"}
        end
        if Window.Flags["BB/Recoil/Enabled"] and (Config.Recoil and Config.Recoil.Default) then
            Config.Recoil.Default.WeaponScale *= Window.Flags["BB/Recoil/WeaponScale"] / 100
            Config.Recoil.Default.CameraScale *= Window.Flags["BB/Recoil/CameraScale"] / 100
            Config.Recoil.Default.RecoilScale *= Window.Flags["BB/Recoil/RecoilScale"] / 100
        end
...
Radical.Utilities.NewThreadLoop(1/10, function()
    CustomizeWeapon(
...
Workspace.Characters.ChildAdded:Connect(function(Child)
    ...
    if Window.Flags["BB/NoClip/Enabled"] then
