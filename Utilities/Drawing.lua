...
local function CalculateFOVRadius(FOVValue)
    ...
    local FOVMultiplier = 70 / CameraFOV
...
function DrawingLibrary.Update(ESP, Target)
    ...
    local Distance, BoxTooSmall = 0, false
    ...
    if OnScreen then
        ...
        if IsWithinReach(GetFlag(Flags, Flag, "/DistanceCheck"), GetFlag(Flags, Flag, "/Distance"), Distance) then
            ...
            if ESP.Drawing.Box.Visible then
                ...
                -- LineLT
                local From = AntiAliasingXY(ScreenPosition.X - (BoxSize.X / 2), ScreenPosition.Y - (BoxSize.Y / 2))
                local To   = AntiAliasingXY(ScreenPosition.X - (BoxSize.X / 2), (ScreenPosition.Y - (BoxSize.Y / 2)) + CornerSize.Y)
                ...
                -- LineTL
                From = AntiAliasingXY(ScreenPosition.X - (BoxSize.X / 2), ScreenPosition.Y - (BoxSize.Y / 2))
                To   = AntiAliasingXY((ScreenPosition.X - (BoxSize.X / 2)) + CornerSize.X, ScreenPosition.Y - (BoxSize.Y / 2))
                ...
                -- LineLB
                From = AntiAliasingXY(ScreenPosition.X - (BoxSize.X / 2), ScreenPosition.Y + (BoxSize.Y / 2))
                To   = AntiAliasingXY(ScreenPosition.X - (BoxSize.X / 2), (ScreenPosition.Y + (BoxSize.Y / 2)) - CornerSize.Y)
                ...
                or Textboxes.Weapon.Visible then
                    ...
                    local Transparency2 = 1 - GetFlag(Flags, Flag, "/Name/Transparency")
                    local Outline = GetFlag(Flags, Flag, "/Name/Outline")

                    if Textboxes.Name.Visible then
                        Textboxes.Name.Outline = Outline
                        Textboxes.Name.Transparency = Transparency2
                        Textboxes.Name.Size = math.floor(Autoscale + 0.5)
                    ...
                    if Textboxes.Health.Visible then
                        ...
                        Textboxes.Health.Transparency = Transparency2
                        Textboxes.Health.Size = math.floor(Autoscale + 0.5)
                        Textboxes.Health.Text = tostring(math.floor(HealthPercent * 100)) .. "%"
                        local HealthPositionX = ESP.Drawing.HealthBar.Main.Visible
                            and ((ScreenPosition.X - (BoxSize.X / 2)) - Textboxes.Health.TextBounds.X - (Thickness + ThicknessAdjust + 5))
                            or  ((ScreenPosition.X - (BoxSize.X / 2)) - Textboxes.Health.TextBounds.X - ThicknessAdjust - 2)
                    ...
                    if Textboxes.Distance.Visible then
                        ...
                        Textboxes.Distance.Transparency = Transparency2
                        Textboxes.Distance.Size = math.floor(Autoscale + 0.5)
                    ...
                    if Textboxes.Weapon.Visible then
                        ...
                        Textboxes.Weapon.Transparency = Transparency2
                        Textboxes.Weapon.Size = math.floor(Autoscale + 0.5)
    ...
    else -- not OnScreen: offscreen arrow
...
function DrawingLibrary.SetupFOV(Flag, Flags)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if Visible then
            ...
            local FOVScale = GetFlag(Flags, Flag, "/FOV/Radius")
...
DrawingLibrary.Connection = RunService.Heartbeat:Connect(function()
    ...
    for Object, ESP in pairs(DrawingLibrary.ObjectESP) do
        ...
        ESP.Target.InTheRange = IsWithinReach(
